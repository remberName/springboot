...
function displayData(userList: Array<{ userName: string, userSex: string, address: string }>) {
	// idによってDomを取得
...
// interface練習
interface Person {
	name: string;// string型なname
	age: number;// number型なage
}
...
function greet(student: Student): Student {
	student.name = "マリア";
	return student;
}

// type練習
type Animal = { name: string; sound: string }; // 文字列型な名前と文字列な声の動物型


//　オブジェクト指向
class Dog {
	// 変更不可なプロパティ
	readonly dogId: number = 110;
	private animal: Animal;
	private age: number;

	//初期化メソッド
	constructor(dogId: number) {
		this.dogId = dogId;
